...
import { usePublicExerciseService, useMuscleGroupService } from '../hooks';
import type { WorkoutPlan, WorkoutDay, ExerciseTarget, DurationUnit, MuscleGroup } from '../domain/models';
...
export default function WorkoutPlanForm({ plan, onSubmit, onCancel }: WorkoutPlanFormProps) {
  ...
  const [formData, setFormData] = useState<Omit<WorkoutPlan, 'id'>>({
    ...
    workoutDayDTOList: plan?.workoutDayDTOList || [],
  });
  ...
  const addWorkoutDay = () => {
    const newDay: WorkoutDay = {
      description: `Day ${formData.workoutDayDTOList.length + 1}`,
      exercises: [],
    };
    setFormData({
      ...formData,
      workoutDayDTOList: [...formData.workoutDayDTOList, newDay],
    });
    setExpandedDayIndex(formData.workoutDayDTOList.length);
  };

  const removeWorkoutDay = (index: number) => {
    setFormData({
      ...formData,
      workoutDayDTOList: formData.workoutDayDTOList.filter((_, i) => i !== index),
    });
  ...
  const updateWorkoutDay = (index: number, field: keyof WorkoutDay, value: any) => {
    const updatedDays = [...formData.workoutDayDTOList];
    updatedDays[index] = { ...updatedDays[index], [field]: value };
    setFormData({ ...formData, workoutDayDTOList: updatedDays });
  };
      ...
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="border-t border-gray-200 pt-6">
            ...
            {formData.workoutDayDTOList.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                No workout days yet. Click "Add Day" to create one.
              </div>
            ) : (
              <div className="space-y-3">
                {formData.workoutDayDTOList.map((day, dayIndex) => (
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-200">
            <button
              type="submit"
              disabled={loading || formData.workoutDayDTOList.length === 0}
