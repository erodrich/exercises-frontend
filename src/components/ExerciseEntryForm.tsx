...
import type {ExerciseLogEntry, ExerciseTarget, MuscleGroup} from '../domain/models';
import ExerciseSetForm from './ExerciseSetForm';
import LatestLogDisplay from './LatestLogDisplay';
import WorkoutPlanTargetDisplay from './WorkoutPlanTargetDisplay';
...
import {useWorkoutPlanService} from '../hooks/useWorkoutPlanService';
...
const ExerciseEntryForm: React.FC<ExerciseEntryFormProps> = ({
    ...
    const [workoutPlanTargets, setWorkoutPlanTargets] = useState<ExerciseTargetWithDay[]>([]);
    ...
    // Fetch workout plan targets when exercise is selected
    useEffect(() => {
        const fetchWorkoutPlanTargets = async () => {
            // Reset previous targets
            setWorkoutPlanTargets([]);

            // Need exercise name selected and user logged in
            if (!entry.exercise.name || !user) {
                return;
            }
            ...
            try {
                ...
                if (result.success && result.data) {
                    ...
                    const activePlans = result.data.filter(plan => plan.active);

                    if (activePlans.length > 0) {
                        // Pick the first active plan (should only be one)
                        ...
                        setWorkoutPlanTargets(matchingTargets);
                    }
                }
            } catch (error) {
                console.error('Failed to fetch workout plan targets:', error);
            }

            setLoadingWorkoutPlan(false);
        };

        fetchWorkoutPlanTargets();
    }, [entry.exercise.name, user, workoutPlanService]);
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Workout Plan Target Display */}
            {entry.exercise.name && (
                <WorkoutPlanTargetDisplay
                    targets={workoutPlanTargets}
                    isLoading={loadingWorkoutPlan}
                />
            )}
